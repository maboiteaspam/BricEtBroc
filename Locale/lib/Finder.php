...
class Finder{
    public static function get_best_locale( $available_locales=array() ){
        if(function_exists("locale_accept_from_http") ){
            if( isset($_SERVER['HTTP_ACCEPT_LANGUAGE']) )
                return locale_accept_from_http($_SERVER['HTTP_ACCEPT_LANGUAGE']);
        }else{
            $retour = self::prefered_language($available_locales);
            return $retour;
        }
    }
    ...
    */
    public static function prefered_language ($available_languages, $http_accept_language="auto") {
        ...
        if ($http_accept_language == "auto" && isset($_SERVER['HTTP_ACCEPT_LANGUAGE']) )
